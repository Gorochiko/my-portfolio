...
function PhoneMockup({ image, title, gradient }: { image?: StaticImageData | string, title: string, gradient: string }) {
  return (
          ...
          <div className={`w-full h-full ${gradient} flex items-center justify-center`}>
            {image ? (
              ...
              <div className="text-center">
                ...
                <p className="text-white/80">Mobile App</p>
              </div>
            )}
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white">
      <div className="container mx-auto px-6 py-8">
            ...
            <div className="relative mb-16">
              <div className="flex justify-center items-center space-x-8">
                ...
                <div className="flex gap-8 justify-center">
                  <PhoneMockup image={mobileProject.image_url_mobile} title={mobileProject.title} gradient="bg-gradient-to-b from-purple-600 via-blue-600 to-emerald-600" />
                  <PhoneMockup image={mobileProject.image_url_screen} title={mobileProject.title} gradient="bg-gradient-to-b from-emerald-600 via-blue-600 to-purple-600" />
                </div>
                {/* Feature Cards */}
              </div>
            </div>
                ...
                {mobileProject.tech && (
                  <div className="grid grid-cols-2 gap-4">
                    {mobileProject.tech.map((tech: string, techIndex: number) => (
                      <div
                        key={techIndex}
                        className="bg-white/10 backdrop-blur-sm rounded-xl p-4 text-center hover:bg-white/20 transition-all duration-200 group"
                        tabIndex={0}
                        aria-label={tech}
                      >
                        <span className="text-white font-medium group-hover:text-emerald-400 transition-colors duration-200">{tech}</span>
                      </div>
                    ))}
                  </div>
                )}
        ...
        {/* Footer */}
        <footer className="text-center text-slate-400 border-t border-white/10 pt-8">
          <p className="mb-2">
